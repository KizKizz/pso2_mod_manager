...
class ItemSwapGridLayout extends StatefulWidget {
  const ItemSwapGridLayout({super.key, required this.modSet});
...
class _ItemSwapGridLayoutState extends State<ItemSwapGridLayout> {
  ...
  Widget build(BuildContext context) {
    return SliverStickyHeader.builder(
        ...
        sliver: ResponsiveSliverGridList(minItemWidth: 350, verticalGridMargin: 5, horizontalGridSpacing: 5, verticalGridSpacing: 5, children: [
          GridTile()
        ]));
  }

  List<ItemSwapCardLayout> modCardFetch() {
    List<ItemSwapCardLayout> modCardList = [];
    for (var item in widget.modSet.setItems) {
      for (var mod in item.mods.where((e) => e.isSet && e.setNames.contains(widget.modSet.setName))) {
        modCardList.add(ItemSwapCardLayout(item: item, mod: mod, setName: widget.modSet.setName));
      }
    }
...
class ItemSwapCardLayout extends StatefulWidget {
  const ItemSwapCardLayout({super.key, required this.itemData});

  final ItemData itemData;

  @override
  State<ItemSwapCardLayout> createState() => _ItemSwapCardLayoutState();
}

class _ItemSwapCardLayoutState extends State<ItemSwapCardLayout> {
  @override
  Widget build(BuildContext context) {
    return CardOverlay(
      paddingValue: 10,
