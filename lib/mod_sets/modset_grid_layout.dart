...
class ModSetGridLayout extends SignalStatefulWidget {
...
class _ModSetGridLayoutState extends State<ModSetGridLayout> {
  ...
  Widget build(BuildContext context) {
    // Refresh
    if (modSetRefreshSignal.value != modSetRefreshSignal.peek()) {
      setState(() {});
    }
    // get data
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: PopupMenuButton(
                          ...
                          color: Theme.of(context).scaffoldBackgroundColor.withAlpha(uiBackgroundColorAlpha.value + 50),
                          padding: EdgeInsets.zero,
                          ...
                          itemBuilder: (BuildContext context) {
                            return [
                              PopupMenuItem(
                                ...
                                child: MenuIconItem(icon: Icons.edit, text: appText.rename, enabled: widget.modSet.setItems.indexWhere((e) => e.applyStatus) == -1),
                              ),
                              PopupMenuItem(
                                ...
                                child: MenuIconItem(icon: Icons.control_point_duplicate, text: appText.duplicate, enabled: widget.modSet.setItems.indexWhere((e) => e.applyStatus) == -1),
                              ),
                              PopupMenuItem(
                                enabled: widget.modSet.setItems.indexWhere((e) => e.applyStatus) == -1,
                                onTap: () async {
                                ...
                                child: MenuIconItem(icon: Icons.delete_forever_outlined, text: appText.delete, enabled: widget.modSet.setItems.indexWhere((e) => e.applyStatus) == -1),
                              ),
                            ];
                          },
                        ),
          ...
          children: [
            ...
            Expanded(
              child: Row(
                ...
                children: [
                  Flexible(
                    child: AutoSizeText(widget.activeMod.modName, maxLines: 1, textAlign: TextAlign.center, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.labelLarge),
                  ),
            ...
            Row(
              ...
              children: [
                Expanded(
                  child: InfoBox(
                    info: appText.dText(
                      widget.item.mods.where((e) => e.isSet && e.setNames.contains(widget.setName)).length > 1 ? appText.numMods : appText.numMod,
                ...
                Expanded(
                  child: InfoBox(
                    info: appText.dText(
                      widget.item.getSubmods().where((e) => e.isSet && e.setNames.contains(widget.setName)).length > 1 ? appText.numVariants : appText.numVariant,
                      widget.item.getSubmods().where((e) => e.isSet && e.setNames.contains(widget.setName)).length.toString(),
                    ),
                    borderHighlight: false,
                  ),
                ),
                Expanded(flex: 2, child: InfoBox(info: appText.dText(appText.numCurrentlyApplied, widget.item.getNumOfAppliedMods().toString()), borderHighlight: false)),
              ],
            ),
            Row(
              spacing: 5,
              children: [
                Visibility(
                  ...
                  child: OutlinedButton(
                    ...
                    child: Text(appText.viewVariants, textAlign: TextAlign.center),
                  ),
                ),
                Expanded(
                  child: OutlinedButton(
