                                  ...
                                  if (snapshot.connectionState == ConnectionState.waiting && customSleevesLoader == null) {
                                    ...
                                    if (snapshot.hasError) {
                                      ...
                                      return FutureBuilder(
                                          future: sleeveDataLoader = originalVitalBackgroundsFetching(context),
                                          builder: ((
                                            BuildContext context,
                                            AsyncSnapshot snapshot,
                                          ) {
...
Future<List<LineStrikeSleeve>> originalVitalBackgroundsFetching(context) async {
  ...
  if (Directory(newTempIconIcePath).existsSync()) {
    ...
    for (var templatePixel in iconTemplate!.data!) {
      if (templatePixel.a > 0) {
        iconImage!.setPixel(templatePixel.x, templatePixel.y, replaceImage!.getPixel(templatePixel.x - 49, templatePixel.y - 16));
      }
    }
