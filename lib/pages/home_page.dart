...
//MODVIEW LIST====================================================================================================================================================================================
  Widget modsView() {
    ...
    //reset state
    if (modViewItem != null && modViewItem!.mods.indexWhere((e) => e.isNew) == -1) {
      modViewListShowNew = false;
    }
    if (modViewItem != null && modViewItem!.mods.indexWhere((e) => e.applyStatus) == -1) {
      modViewListShowApplied = false;
    }

    //Filters
    if (modViewListVisible && modViewItem != null) {
      if (modViewListShowNew && !modViewListShowApplied) {
        stateModFilters.addAll(modViewItem!.mods.where((e) => e.isNew));
      } else if (modViewListShowApplied && !modViewListShowNew) {
        stateModFilters.addAll(modViewItem!.mods.where((e) => e.applyStatus));
      } else if (modViewListShowNew && modViewListShowApplied) {
        stateModFilters = modViewItem!.mods.where((e) => e.isNew || e.applyStatus).toList();
      } else {
        stateModFilters = modViewItem!.mods;
      }

      if (isFavListVisible && !isModViewFromApplied) {
        modViewMods = stateModFilters.where((e) => e.isFavorite).toList();
      } else if (searchTextController.value.text.toLowerCase().isNotEmpty && !isModViewFromApplied && itemModSearchMatchesCheck(modViewItem!, searchTextController.value.text) > 0) {
                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        if (modViewListVisible)
                          Wrap(spacing: 2.5, runSpacing: 2.5, children: [
                            ...
                            //new count
                            Visibility(
                              visible: modViewItem!.mods.indexWhere((element) => element.isNew) != -1,
                              child: Container(
                                padding: const EdgeInsets.only(left: 2, right: 2, bottom: 1),
                                decoration: BoxDecoration(
                                  border: Border.all(color: Theme.of(context).primaryColorLight),
                            ...
                            Visibility(
                              visible: modViewItem!.mods.indexWhere((element) => element.applyStatus) != -1,
                              child: InkWell(
                                onTap: () async {
                                ...
                                ? ConstrainedBox(
                                    ...
                                    child: Container(
                                      ...
                                      child: FlutterCarousel.builder(
                                        options: previewCarouseOption(context),
                                      ...
                                      MenuAnchor(
                                          ...
                                          menuChildren: [
                                            ...
                                            MenuItemButton(
                                              leadingIcon: Icon(
                                                Icons.delete_forever_outlined,
                                                color: curMod.applyStatus || curMod.submods.first.location == curMod.location && Directory(curMod.location).listSync().whereType<Directory>().isNotEmpty
                                                        ...
                                                        if (modViewMods.length < 2) {
                                                          deleteItemFromModMan(modViewItem!.location).then((value) async {
                                                            ...
                                                            ScaffoldMessenger.of(context).showSnackBar(
                                                                snackBarMessage(context, '${curLangText!.uiSuccess}!', uiInTextArg(curLangText!.uiSuccessfullyRemovedXFromModMan, removedName), 3000));
                                                            previewModName = '';
                                                            previewImages.clear();
                                                            saveModdedItemListToJson();
                                                            setState(() {});
                                                          ...
                                                          onTap: () async {
                                                            ...
                                                            Future.delayed(Duration(milliseconds: unapplyButtonsDelay), () {
                                                              restoreOriginalFilesToTheGame(context, curMod.submods.first.modFiles).then((unappliedModFiles) async {
                                                                ...
                                                                if (modViewMods.indexWhere((element) => element.applyStatus) == -1) {
...
CarouselOptions previewCarouseOption(context) {
