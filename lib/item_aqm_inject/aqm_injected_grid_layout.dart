  ...
  Widget build(BuildContext context) {
    ...
    if (injectedItemSearchTextController.value.text.isEmpty) {
      ...
      displayingAqmInjectedItem = widget.injectedItemList
          .where(
            (e) =>
                e.getName().toLowerCase().contains(injectedItemSearchTextController.value.text.toLowerCase()) ||
                p.basenameWithoutExtension(e.hqIcePath).contains(injectedItemSearchTextController.value.text.toLowerCase()) ||
                p.basenameWithoutExtension(e.lqIcePath).contains(injectedItemSearchTextController.value.text.toLowerCase()) ||
                p.basenameWithoutExtension(e.iconIcePath).contains(injectedItemSearchTextController.value.text.toLowerCase()),
          )
          .toList();
    }
                      ...
                      title: Row(
                        ...
                        children: [
                          ...
                          Column(
                            ...
                            children: [
                              Text(displayingAqmInjectedItem[index].getName(), style: const TextStyle(fontWeight: FontWeight.w500)),
                              Row(
                                spacing: 5,
                                mainAxisAlignment: MainAxisAlignment.start,
                                children: [
                                  Visibility(
                                    ...
                                    child: InfoBox(info: appText.aqmInjected, borderHighlight: false),
                                  ),
                                  Visibility(
                                    visible: displayingAqmInjectedItem[index].isBoundingRemoved!,
                                    child: InfoBox(info: appText.boundingRemoved, borderHighlight: false),
                                  ),
                                ],
                              ),
                              Visibility(
                                visible: displayingAqmInjectedItem[index].isAqmReplaced!,
                                child: Text(appText.dText(appText.injectedAQMFile, p.basename(displayingAqmInjectedItem[index].injectedAQMFilePath!)), style: Theme.of(context).textTheme.labelMedium),
                              ),
                            ],
                          ),
                        ],
                                      ...
                                      child: OutlinedButton(
                                        onPressed: () async {
                                          ...
                                          if (result && !displayingAqmInjectedItem[index].isBoundingRemoved!) {
                                            ...
                                            displayingAqmInjectedItem[index].injectedHqIceMd5 = await File(
                                              pso2binDirPath + p.separator + displayingAqmInjectedItem[index].hqIcePath.replaceAll('/', p.separator),
                                            ).getMd5Hash();
                                            displayingAqmInjectedItem[index].injectedLqIceMd5 = await File(
                                              pso2binDirPath + p.separator + displayingAqmInjectedItem[index].lqIcePath.replaceAll('/', p.separator),
                                            ).getMd5Hash();
                                            modAqmInjectingRefresh.value = 'Removed AQM and restored ${displayingAqmInjectedItem[index].getName()}';
                                          }
