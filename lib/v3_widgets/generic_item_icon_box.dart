        ...
        builder: (context) => Card(
          ...
          child: FlutterCarousel(
            options: FlutterCarouselOptions(
              autoPlay: widget.iconImagePaths.length > 1 && itemIconSlides.value ? true : false,
              autoPlayInterval: const Duration(seconds: 2),
              disableCenter: true,
              viewportFraction: 1.0,
              height: double.infinity,
              floatingIndicator: true,
              enableInfiniteScroll: widget.iconImagePaths.length > 1 && itemIconSlides.value ? true : false,
              indicatorMargin: 2,
            ...
            items: widget.iconImagePaths.isNotEmpty
                ? widget.iconImagePaths
                      .map(
                        (e) => widget.isNetwork
                            ? Image.network(
                                'https://raw.githubusercontent.com/KizKizz/pso2ngs_file_downloader/main$e',
                                filterQuality: FilterQuality.high,
                                fit: BoxFit.cover,
                                errorBuilder: (context, error, stackTrace) => Image.asset('assets/img/placeholdersquare.png', filterQuality: FilterQuality.high, fit: BoxFit.cover),
                              )
                            : Image.file(File(e), filterQuality: FilterQuality.high, fit: BoxFit.cover),
                      )
                      .toList()
                : [Image.asset('assets/img/placeholdersquare.png', filterQuality: FilterQuality.high, fit: BoxFit.cover)],
          ),
        ),
      ),
    );
