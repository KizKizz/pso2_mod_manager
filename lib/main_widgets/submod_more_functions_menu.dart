...
import 'package:pso2_mod_manager/item_aqm_inject/mod_aqm_select_popup.dart';
        ...
        itemBuilder: (BuildContext context) {
          return [
            PopupMenuItem(
              ...
              child: MenuIconItem(icon: Icons.list_alt_outlined, text: appText.modSets, enabled: true),
            ),
            PopupMenuItem(
              onTap: () async {
              ...
              child: ModManTooltip(
                ...
                child: MenuIconItem(icon: Icons.add_location_alt_outlined, text: appText.setApplyLocations, enabled: true),
              ),
            ),
            PopupMenuItem(
              onTap: () {
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(appText.applyHQFilesOnly),
                  Visibility(visible: widget.submod.applyHQFilesOnly!, child: const Icon(Icons.check)),
                ],
              ),
            ),
            ...
            PopupMenuItem(
              ...
              child: MenuIconItem(icon: Icons.swap_horizontal_circle_outlined, text: appText.swapToAnotherItem, enabled: true),
            ),
            PopupMenuItem(
              enabled: false,
              child: MenuIconItem(icon: Icons.file_present, text: appText.cmx, enabled: true),
            ),
            PopupMenuItem(
              ...
              PopupMenuItem(
                ...
                onTap: () async {
                  await modAqmSelectPopup(
                    context,
                    widget.submod,
                    Directory(modCustomAqmsDirPath).listSync().whereType<File>().where((e) => p.extension(e.path) == '.aqm').map((e) => e.path).toList(),
                    [],
                    selectedCustomAQMFilePath,
                    [],
                  );
            ...
            if (widget.submod.customAQMInjected!)
              PopupMenuItem(
                ...
                child: MenuIconItem(icon: Icons.auto_fix_off, text: appText.removeCustomAQMs, enabled: true),
              ),
            const PopupMenuItem(height: 0, enabled: false, child: PopupMenuDivider(height: 5)),
            PopupMenuItem(
              onTap: () async {
              ...
              child: MenuIconItem(icon: Icons.preview_outlined, text: appText.addPreviews, enabled: true),
            ),
            PopupMenuItem(
              onTap: () => modExportSequence(context, ExportType.submods, widget.item, widget.mod, widget.submod, null),
