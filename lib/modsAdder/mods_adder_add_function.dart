...
List<Mod> newModsFetcher(String itemPath, String cateName, List<Directory> newModFolders) {
  ...
  if (iceFilesInItemDir.isNotEmpty) {
    ...
    //add to submod
    SubMod subModInItemDir = SubMod(p.basename(itemPath), p.basename(itemPath), p.basename(itemPath), cateName, itemPath, false, DateTime(0), 0, false, false, false, false, false, -1, -1, [], [], modPreviewImages,
...
List<SubMod> newSubModFetcher(String modPath, String cateName, String itemName) {
  ...
  if (filesInMainModDir.isNotEmpty) {
    ...
    submods.add(SubMod(p.basename(modPath), p.basename(modPath), itemName, cateName, modPath, false, DateTime(0), 0, true, false, false, false, false, -1, -1, [], [], modPreviewImages, modPreviewVideos, [], modFiles));
  }
  ...
  for (var dir in foldersInModDir) {
    ...
    parentPaths.removeWhere((element) => element.isEmpty);
    submods.add(SubMod(parentPaths.join(' > '), p.basename(modPath), itemName, cateName, dir.path, false, DateTime(0), 0, true, false, false, false, false, -1, -1, [], [], modPreviewImages, modPreviewVideos, [], modFiles));
  }
