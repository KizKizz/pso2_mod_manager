...
Future<List<ModFile>> modsLoader() async {
  final allFiles =
      Directory(modsDirPath).listSync(recursive: true).whereType<File>();
  ...
  for (var iceFile in iceFiles) {
    ...
    for (var element in iceFilePathSplit) {
      if (element == 'Mods') {
        categoryName = iceFilePathSplit[
            iceFilePathSplit.indexWhere((e) => e == element) + 1];
        categoryPath = iceFile.path.split(categoryName).first + categoryName;
      }
      if (element == categoryName) {
        modName = iceFilePathSplit[
            iceFilePathSplit.indexWhere((e) => e == categoryName) + 1];
    ...
    //Image helper
    for (var imgFile in Directory(iceFile.parent.path)
        .listSync(recursive: false)
        .whereType<File>()) {
      if (p.extension(imgFile.path) == '.jpg' ||
          p.extension(imgFile.path) == '.png') {
        imgFiles.add(imgFile);
      }
    }

    //Vids helper
    for (var vidFile in Directory(iceFile.parent.path)
        .listSync(recursive: false)
        .whereType<File>()) {
      if (p.extension(vidFile.path) == '.mp4' ||
          p.extension(vidFile.path) == '.webm') {
    ...
    if (imgFiles.isEmpty || vidFiles.isEmpty) {
      List<String> filePathSplit =
          iceFile.path.split('$modPath\\').last.split(s);
    ...
    ModFile newModFile = ModFile(
        ...
        modName,
        iceFile.path,
        iceFilePathSplit.last,
        iceParents,
        '',
        '',
        getImagesList(imgFiles),
        false,
        true,
        false,
        false,
    ...
    var jsonModFile = modFilesFromJson
        .firstWhere((e) => e.icePath == newModFile.icePath, orElse: () {
      return ModFile(
...
List<ModCategory> categories(List<ModFile> allModFiles) {
  ...
  for (var modFile in allModFiles) {
    ...
    List<File> imgFiles = [];
    final filesGet =
        Directory(modFile.modPath).listSync(recursive: false).whereType<File>();
    ...
    if (categories.isEmpty ||
        categories.indexWhere((e) => e.categoryName == modFile.categoryName) ==
            -1) {
      ...
      List<ModFile> sameMod = newCategory.allModFiles
          .where((element) => element.modName == modFile.modName)
          .toList();
      List<String> parentsList = [];
      if (sameMod.isNotEmpty) {
        parentsList.add(modFile.iceParent);
        for (var file in sameMod) {
          if (parentsList.indexWhere((element) => element == file.iceParent) ==
              -1) {
      ...
      ModCategory matchedCategory =
          categories.firstWhere((e) => e.categoryName == modFile.categoryName);

      if (matchedCategory.itemNames
              .indexWhere((element) => element == modFile.modName) !=
          -1) {
        if (modFile.isApplied &&
            matchedCategory.itemNames
                    .indexWhere((element) => element == modFile.modName) ==
                -1) {
          matchedCategory.numOfApplied[matchedCategory.itemNames
              .indexWhere((element) => element == modFile.modName)]++;
        }
        int curItemIndex = 0;
        ...
        List<ModFile> sameMod = matchedCategory.allModFiles
            .where((element) => element.modName == modFile.modName)
            .toList();
        List<String> parentsList = [];
        if (sameMod.isNotEmpty) {
          ...
          for (var file in sameMod) {
            if (parentsList
                    .indexWhere((element) => element == file.iceParent) ==
                -1) {
              parentsList.add(file.iceParent);
            }
          }
  ...
  //Add Empty Category
  final cateDirs =
      Directory(modsDirPath).listSync(recursive: false).whereType<Directory>();
  for (var dir in cateDirs) {
    final emptyCateDirs = dir.listSync(recursive: false);
    if (emptyCateDirs.isEmpty) {
      categories.add(
          ModCategory(dir.path.split(s).last, dir.path, [], [], 0, [], [], []));
  ...
  for (var mod in favModListGet) {
    if (modNames.isEmpty ||
        modNames.indexWhere((element) => element == mod.modName) == -1) {
  ...
  for (var name in modNames) {
    ...
    for (var parent in parents) {
      List<ModFile> sameParent =
          sameMods.where((element) => element.iceParent == parent).toList();
...
//Search result
List<ModCategory> searchFilterResults(
    List<ModCategory> paramCateList, String searchText) {
  List<ModCategory> resultList = [];

  for (var cate in paramCateList) {
    if (cate.categoryName != 'Favorites') {
      List<String> itemNameResults = [];
      if (cate.categoryName == 'Basewears' ||
          cate.categoryName == 'Outerwears' ||
          cate.categoryName == 'Innerwears' ||
          cate.categoryName == 'Setwears') {
        itemNameResults = cate.itemNames
            .where((element) => element
                .replaceRange(element.length - 4, null, '')
                .toLowerCase()
      ...
      if (itemNameResults.isNotEmpty) {
        ModCategory newCate = ModCategory(cate.categoryName, cate.categoryPath,
            [], [], itemNameResults.length, [], [], []);
        for (var itemName in itemNameResults) {
          ...
          List<ModFile> modsInItems = cate.allModFiles
              .where((element) => element.modName == itemName)
              .toList();
        ...
        List<ModFile> modFileResults = cate.allModFiles
            .where((element) =>
                element.iceName
                    .toLowerCase()
                    .contains(searchText.toLowerCase()) ||
        ...
        if (modFileResults.isNotEmpty) {
          ...
          for (var element in modFileResults) {
            if (itemNamesfromMods.indexWhere((e) => e == element.modName) ==
                -1) {
              itemNamesfromMods.add(element.modName);
            }
          }

          ModCategory newCate = ModCategory(cate.categoryName,
              cate.categoryPath, [], [], itemNamesfromMods.length, [], [], []);
          for (var itemName in itemNamesfromMods) {
            ...
            for (var element in modFileResults
                .where((element) => element.modName == itemName)) {
              if (parentsInItems.indexWhere((e) => e == element.iceParent) ==
                  -1) {
                parentsInItems.add(element.iceParent);
              }
            }
            int numofMod = parentsInItems.length;
            int numofApplied = modFileResults
                .where((element) => element.isApplied == true)
                .length; // need to change
            List<ModFile> modsInItems = modFileResults
                .where((element) => element.modName == itemName)
                .toList();
...
//Mod List
Future<List<List<ModFile>>> getModFilesByCategory(
    List<ModFile> allModFiles, String modName) async {
...
Future<List<List<ModFile>>> getAppliedModsList() async {
  ...
  List<ModFile> actualAppliedMods =
      allModFiles.where((element) => element.isApplied == true).toList();
  List<String> parentPathsList = [];

  for (var file in actualAppliedMods) {
    if (parentPathsList.indexWhere((element) =>
            element == file.icePath.replaceFirst(file.iceName, '')) ==
        -1) {
  ...
  for (var path in parentPathsList) {
    actualAppliedMods.addAll(allModFiles
        .where((element) =>
            element.icePath.replaceFirst(element.iceName, '') == path &&
  ...
  appliedList
      .sort(((a, b) => b.first.appliedDate.compareTo(a.first.appliedDate)));
  return appliedList;
}
...
Future<List<ModSet>> getSetsList() async {
  ...
  if (returnSetsList.isEmpty &&
      File(modSetsSettingsPath).readAsStringSync().isNotEmpty) {
