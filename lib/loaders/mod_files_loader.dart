...
Future<List<Category>> modFilesLoader(String modsDirPath) async {
  ...
  for (var folder in allDataFolders) {
    modsDirFolderPaths.add(Uri.directory(folder.path).toFilePath());
  }
  ...
  for (var cateDir in dirsInModsDir) {
    List<String> curCateItemPaths = modsDirFolderPaths.where((element) => element.contains(cateDir.path)).toList();
    for (var element in curCateItemPaths) {
      print(element);
    }
    ...
    for (var itemDir in curCateItemPaths) {
      //   //Get item icon
      String itemIcon = '';
      final imagesInItemDir = Directory('').listSync(recursive: false).whereType<File>().where((element) => p.extension(element.path) == '.jpg' || p.extension(element.path) == '.png');
      ...
      //     final videosInModDir = Directory(modDir.path).listSync(recursive: false).whereType<File>().where((element) => p.extension(element.path) == '.webm' || p.extension(element.path) == '.mp4');
      ...
      //         //List<String> ogIceFileLocations = ogDataFilePaths.where((element) => XFile(element).name == XFile(modFile.path).name).toList();
      //         // if (File(Uri.file('$modManPso2binPath/data/win32/${XFile(modFile.path).name}').toFilePath()).existsSync()) {
      //         //   ogIceFileLocations.add(Uri.file('$modManPso2binPath/data/win32/${XFile(modFile.path).name}').toFilePath());
      //         // }
      //         // if (File(Uri.file('$modManPso2binPath/data/win32_na/${XFile(modFile.path).name}').toFilePath()).existsSync()) {
      //         //   ogIceFileLocations.add(Uri.file('$modManPso2binPath/data/win32_na/${XFile(modFile.path).name}').toFilePath());
      //         // }
      //         // List<File> rebootDataIces = [];
      //         // rebootDataIces.addAll(Directory(Uri.file('$modManPso2binPath/data/win32reboot').toFilePath()).listSync(recursive: true).whereType<File>());
      //         // rebootDataIces.addAll(Directory(Uri.file('$modManPso2binPath/data/win32reboot_na').toFilePath()).listSync(recursive: true).whereType<File>());
      //         // final rebootIces = rebootDataIces.where((element) => XFile(element.path).name == XFile(modFile.path).name);
...
//Path helper
String getSubDirName(String path, String parentPath) {
  final half = path.split(parentPath);
  print(half[1]);
  return half[1];
}
